...
function formatMoneyWithoutCurrency(value: number | string | null | undefined) {
...
async function main() {
  ...
  console.log('Experiment B2 — PDFKit production-like formatting');
  ...
  doc.moveDown();
  doc.text(`Opening balance: ${formatMoneyWithoutCurrency(1000000)}`);
  doc.moveDown();
  ...
  // simulate production-like rows with running balance
  let running = 1000000; // opening
  ...
  for (let i = 1; i <= maxRows; i++) {
    ...
    const amount = (i % 100) + 1; // small varied amount
    ...
    // production-like single-line text with tabs
